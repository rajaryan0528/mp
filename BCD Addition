...
MESS0 DB 10,13,"ENTER THE FIRST NUMBER:$" ; 10 is the ASCII control code for line feed while 13 is the 
...
AGAINS: 
MOV AH,01 
INT 21H ; input the number as AH =01 
CMP AL,'A' 
JGE L51 
SUB AL,30H ; convert the hexadecimal digits into its equivalent ASCII
JMP L61 
...
ADD AL,BL 
DAA ; represented number in 8-bit packed BCD code 
MOV BL,AL 
ADC AH,BH ; Add with carry BH to AH 
MOV AL,AH 
DAA ; represented number in 8-bit packed BCD code 
MOV BH,AL 
MOV WORD PTR C,BX 
...
ADC AL,BL ; Add with carry BL to AL 
DAA ; represented number in 8-bit packed BCD code 
MOV BL,AL 
ADC AH,BH ; ; Add with carry BH to AH 
MOV AL,AH 
...
DEC DH 
CMP DH,0 
MOV BX,WORD PTR C ; replace only the lowest word (2 bytes) at the "C" address 
JNZ L1 
MOV AH,4CH 
